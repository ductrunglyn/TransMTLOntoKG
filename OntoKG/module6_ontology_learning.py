...
def reduce_with_umap(
    ...
    if HAS_SKLEARN:
        LOGGER.warning("UMAP không khả dụng — dùng PCA thay thế.")
        pca = PCA(n_components=min(n_components, matrix.shape[1]),
                  random_state=random_state)
        return pca.fit_transform(matrix)

    LOGGER.warning("Cả UMAP và sklearn đều không có — dùng embedding gốc.")
    return matrix
...
def cluster_with_hdbscan(
    ...
    if HAS_SKLEARN:
        ...
        LOGGER.warning("HDBSCAN không khả dụng — dùng KMeans(k=20) thay thế.")
